...
class TestGrlcHome:
    ...
    def test_home(self, client):
        ...
        assert (
            "grlc generates RESTful APIs using SPARQL queries stored in GitHub repositories"
            in body
        )
...
class TestGrlcSpec:
    ...
    def test_subdir_commit(self, mock_loader, client):
        ...
        rv = client.get(
            "/api-git/testuser/testrepo/subdir/testsubdir/commit/local/swagger"
        )
        self.validate(rv)
...
class TestGrlcExec:
    ...
    def test_subdir(self, mock_dispatch, mock_loader, client):
        ...
        rv = client.get(
            "/api-git/testuser/testrepo/subdir/testsubdir/query_name",
            headers={"accept": "application/json"},
        )
        self.validate(rv)
    ...
    def test_commit(self, mock_dispatch, mock_loader, client):
        ...
        rv = client.get(
            "/api-git/testuser/testrepo/commit/local/query_name",
            headers={"accept": "application/json"},
        )
        self.validate(rv)
    ...
    def test_subdir_commit(self, mock_dispatch, mock_loader, client):
        ...
        rv = client.get(
            "/api-git/testuser/testrepo/subdir/testsubdir/commit/local/query_name",
            headers={"accept": "application/json"},
        )
        self.validate(rv)
    ...
    def test_local(self, mock_dispatch, client):
        ...
        rv = client.get("/api-local/query_name", headers={"accept": "application/json"})
        self.validate(rv)
    ...
    def test_url(self, mock_dispatch, mock_get, client):
        ...
        rv = client.get(
            "/api-url/<query_name>?specUrl=http://example.org/url.yml",
            headers={"accept": "application/json"},
        )
        self.validate(rv)
