...
class TestGQuery(unittest.TestCase):
    ...
    def test_guess_endpoint(self):
        with self.app.test_request_context(
            "/?endpoint=http://url-endpoint/from-url/sparql"
        ):
            endpoint, _ = gquery.guess_endpoint_uri("", self.loader)
            self.assertIn("from-url", endpoint, "Should match endpoint given in url")

        with self.app.test_request_context("/"):
            endpoint, _ = gquery.guess_endpoint_uri("", self.loader)
            self.assertIn(
                "from-file", endpoint, "Should match endpoint in endpoint.txt"
            )

            rq, _ = self.loader.getTextForName("test-rq")
            endpoint, _ = gquery.guess_endpoint_uri(rq, self.loader)
            self.assertIn(
                "from-decorator", endpoint, "Should match endpoint in test-rq.rq"
            )
    ...
    def test_get_parameters(self):
        ...
        for paramName, param in params.items():
            ...
            if "_xsd_date" in orig:
                self.assertEqual(
                    param["datatype"], "xsd:date", "Should be type xsd:date"
                )
        ...
        self.assertEqual(
            params["o6"]["datatype"], "xsd:integer", "o6 should be a integer"
        )
    ...
    def test_get_enumeration(self, mock_get):
        ...
        mock_get.return_value.json.return_value = {
            "results": {"bindings": [{"o1": {"value": "v1"}}, {"o1": {"value": "v2"}}]}
        }

        rq, _ = self.loader.getTextForName("test-rq")
        metadata = {"enumerate": "o1"}
        enumeration = gquery.get_enumeration(
            rq, "o1", "http://mock-endpoint/sparql", metadata
        )
    ...
    def test_get_static_enumeration(self):
        ...
        enumeration = gquery.get_enumeration(
            rq, "o", "http://mock-endpoint/sparql", metadata
        )
        self.assertIsInstance(enumeration, list, "Should return a list of values")
        self.assertEqual(len(enumeration), 2, "Should have two elements")
    ...
    def test_get_yaml_decorators(self):
        ...
        self.assertIn("pagination", decorators, "Should have a pagination field")
    ...
    def test_get_metadata(self):
        ...
        self.assertEqual(metadata["type"], "SelectQuery", "Should be type SelectQuery")
        self.assertIsInstance(
            metadata["variables"], list, "Should be a list of variables"
        )
        for var in metadata["variables"]:
            self.assertIsInstance(
                var, rdflib.term.Variable, "Should be of type Variable"
            )
    ...
    def test_paginate_query(self):
        ...
        self.assertNotIn("LIMIT", rq, "Original query should not contain LIMIT keyword")
        self.assertIn("LIMIT", rq_pag, "Paginated query should contain LIMIT keyword")
        self.assertNotIn(
            "OFFSET", rq, "Original query should not contain OFFSET keyword"
        )
        self.assertIn("OFFSET", rq_pag, "Paginated query should contain OFFSET keyword")
    ...
    def test_rewrite_query(self):
        ...
        for pName, pValue in parameters.items():
            ...
            self.assertNotIn(
                pName,
                rq_rw,
                "Rewritten query should not contain original parameter name",
            )
            self.assertNotIn(
                pValue["name"],
                rq,
                "Original query should not contain replacement parameter value",
            )
            self.assertIn(
                pValue["name"],
                rq_rw,
