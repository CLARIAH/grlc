...
import grlc_fork.utils as utils
...
def get_repo_info(loader, sha, prov_g):
    ...
    glogger.debug("Entry in function: swagger.get_repo_info")
    ...
    glogger.debug("Exit of function: swagger.get_repo_info")
...
def get_path_for_item(item):
    ...
    the given item."""
    glogger.debug("Entry in function: swagger.get_path_for_item")
    ...
    glogger.debug("Exit of function: swagger.get_path_for_item")
    return item_path


def build_spec(user, repo, subdir=None, query_url=None, sha=None, prov=None, extraMetadata=[], branch='main'):
    """Build grlc specification for the given github user / repo."""
    glogger.debug("Entry in function: swagger.build_spec")
    loader = utils.getLoader(user, repo, subdir, query_url, sha=sha, prov=prov, branch=branch)
    ...
    glogger.debug("Exit of function: swagger.build_spec")
...
def process_tpf_query_text(query_text, raw_repo_uri, call_name, extraMetadata):
    ...
    glogger.debug("Entry in function: swagger.process_tpf_query_text")
    ...
    glogger.debug("Exit of function: swagger.process_tpf_query_text")
    return item
...
def process_sparql_query_text(query_text, loader, call_name, extraMetadata):
    ...
    glogger.debug("Entry in function: swagger.process_sparql_query_text")
    ...
    glogger.debug("Exit of function: swagger.process_sparql_query_text")
    return item
...
def packItem(call_name, method, tags, summary, description, params, query_metadata, extraMetadata):
    ...
    glogger.debug("Entry in function: swagger.packItem")
    item = {
        'call_name': call_name,
        'method': method,
        'tags': tags,
    ...
    glogger.debug("Exit of function: swagger.packItem")
    return item
